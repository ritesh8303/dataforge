            ...
            for story_id in story_ids:
                try:
                    ...
                    if item and not item.get("deleted") and not item.get("dead"):
                        ...
                        match = re.search(
                            r"^(.*?)\b(is hiring|Is Hiring|hiring|Hiring)\b\s*(a|an)?\s*(.*)$",
                            clean_title_text,
                            re.IGNORECASE,
                        )
                        if match:
                        ...
                        if loc_match:
                            ...
                            remote = "remote" in location_str.lower() or (
                                "onsite" not in location_str.lower() and "on-site" not in location_str.lower()
                            )
                        else:
...
class BerlinStartupJobsFetcher:
    ...
    FEED_URLS = ["https://berlinstartupjobs.com/feed/", "https://berlinstartupjobs.com/engineering/feed/"]
            ...
            try:
                ...
                for item in items:
                    ...
                    if "remote" in title_lower or "home office" in title_lower or "home-office" in title_lower:
                        ...
                        "remote" in desc_lower
                        or "home office" in desc_lower
                        or "work from home" in desc_lower
                        or "mobiles arbeiten" in desc_lower
                    ):
                        remote = True
