...
class TestLambdaHandler:
    ...
    def test_successful_processing(self, mock_boto_client, mock_engine_class):
        ...
        # Mock event
        event = {"Records": [{"s3": {"bucket": {"name": "test-bucket"}, "object": {"key": "test-key.json"}}}]}

        context = MagicMock()

        # Mock S3 get_object response to return valid JSON content
        mock_json_content = json.dumps({"records": [{"id": 1, "name": "Test", "value": 100}]})
        mock_s3.get_object.return_value = {"Body": BytesIO(mock_json_content.encode("utf-8"))}
    ...
    def test_processing_multiple_records(self, mock_boto_client, mock_engine_class):
        ...
        event = {
            "Records": [
                {"s3": {"bucket": {"name": "bucket1"}, "object": {"key": "key1.json"}}},
                {"s3": {"bucket": {"name": "bucket2"}, "object": {"key": "key2.json"}}},
            ]
        }
