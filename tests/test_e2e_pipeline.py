...
def test_pipeline_e2e():
    ...
    response = lambda_client.invoke(
        FunctionName="dataforge-ingestor", InvocationType="RequestResponse", Payload=json.dumps({})
    )
    ...
    try:
        objects = s3_client.list_objects_v2(Bucket="dataforge-bronze-dev-eu-central-1", Prefix="arbeitnow/")
    ...
    response = lambda_client.invoke(
        FunctionName="dataforge-transformer", InvocationType="RequestResponse", Payload=json.dumps(event)
    )
    ...
    try:
        objects = s3_client.list_objects_v2(
            Bucket="dataforge-silver-dev-eu-central-1", Prefix="cleaned/jobs_history.parquet"
        )
        if objects.get("KeyCount", 0) == 0:
    ...
    try:
        objects = s3_client.list_objects_v2(Bucket="dataforge-gold-dev-eu-central-1")
        if objects.get("KeyCount", 0) == 0:
    ...
    try:
        ...
        if len(enabled) != 7:
            ...
            return False
        print("   ✅ All 7 schedules enabled (including scheduled transformer, HN, and Berlin Startup Jobs ingestors)")
    except Exception as e:
        print(f"   ❌ FAILED: {e}")
        return False
